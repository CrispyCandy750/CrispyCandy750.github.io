...
function autodiscoveryInject(data) {
  ...
  if (typeof type === 'string') {
    autodiscoveryTag += `<link rel="alternate" href="${url_for.call(this, path)}" `="" +="" `title="${config.title}" type="application/${type.replace(/2$/, '')}+xml">\n`;
  } else {
    type.forEach((feedType, i) =&gt; {
      autodiscoveryTag += `<link rel="alternate" href="${url_for.call(this, path[i])}" `="" +="" `title="${config.title}" type="application/${feedType.replace(/2$/, '')}+xml">\n`;
    });
  }
